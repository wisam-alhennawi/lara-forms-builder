    ...
    <fieldset class="lfb-fieldset">
        <div class="lfb-fieldset-container">
            <div class="lfb-fieldset-item">
                <input wire:key="form-checkbox-component-{{ md5($key) }}" type="checkbox" id="{{ $key }}" name="{{ $key }}" wire:model="{{ $key }}" class="lfb-checkbox @if(isset($readOnly) && $readOnly) lfb-readonly @endif"
                @if (isset($mode) && ($mode == 'view' || $mode == 'confirm')) disabled @endif @if(isset($readOnly) && $readOnly) readonly disabled @endif>
